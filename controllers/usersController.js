...
class UserController {
  ...
  static async findOneInvestor(req, res, next) {
    try {
      ...
      res.status(200).json({id: data.id, username: data.username, email: data.email, phoneNumber: data.phoneNumber});
    } catch (err) {
      next(err);
    }
  }
  ...
  static async findOneFarmer(req, res, next) {
    try {
      ...
      res.status(200).json({id: data.id, username: data.username, email: data.email, phoneNumber: data.phoneNumber, address: data.address});
    } catch (err) {
      next(err);
    }
  }
